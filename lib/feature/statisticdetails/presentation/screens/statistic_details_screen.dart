                    ...
                    children: [
                      GridView.count(
                        ...
                        children: List.generate(statisticListData.length, (pos)
                        {
                          ...
                          return
                            // statisticListData[pos].value.contains("pdf") ||  statisticListData[pos].value.contains(".com") ? const SizedBox():
                            StatisticDetailsItem(isMenuOpenMap: isMenuOpenMap,
                              name: Helper.getCurrentLocal() == 'AR' ? statisticListData[pos].arName : statisticListData[pos].enName,
                              data: state.data ,
                              sort: statisticListData[pos].sort ,
                              columnName: statisticListData[pos].columnName ,
                              icon: statisticListData[pos].iconSvg ,
                              statisticListData: statisticListData ,
                              pos: pos , uniqueId: widget.uniqueId ,

                              id: statisticListData[pos].id,
                              userColor: statisticListData[pos].userColor == '' ? statisticListData[pos].color : statisticListData[pos].userColor
                              ,itemName: Helper.getCurrentLocal() == 'AR' ? statisticListData[pos].arName : statisticListData[pos].enName,
                              itemValue: statisticListData[pos].value,);
                        }),
                      ),
                    ...
                    children: [
                      GridView.count(
                        ...
                        children: List.generate(statisticListData.length, (pos)
                        {
                          //isMenuOpenMap.clear();
                          isMenuOpenMap.putIfAbsent(statisticListData[pos].columnName, () => false);
                          return StatisticDetailsItem(isMenuOpenMap: isMenuOpenMap,
                            data: state.data ,
                            name: Helper.getCurrentLocal() == 'AR' ? statisticListData[pos].arName : statisticListData[pos].enName,
                            sort: state.statisticList[pos].sort ,
                            columnName: state.statisticList[pos].columnName ,
                            icon: state.statisticList[pos].iconSvg ,
                            statisticListData: state.statisticList ,
                            pos: pos , uniqueId: widget.uniqueId ,
                            id: state.statisticList[pos].id,
                            userColor: state.statisticList[pos].userColor == '' ? state.statisticList[pos].color : state.statisticList[pos].userColor ,
                            itemName: Helper.getCurrentLocal() == 'AR' ? statisticListData[pos].arName : statisticListData[pos].enName,itemValue: statisticListData[pos].value,);
                        }),
                      ),
                      state.data.charts.isEmpty? const SizedBox() :  ListView.builder(shrinkWrap: true, physics: const ClampingScrollPhysics(),itemCount:state.data.charts.length  , itemBuilder: (ctx , pos){
                        return ChartWidget(englishName: state.data.charts[pos].chartSetting.enName, araibicName: state.data.charts[pos].chartSetting.arName , chartsColors: state.data.charts[pos].finalApiData.color , precent: state.data.charts[pos].finalApiData.percent , chartData: state.data.charts[pos].chartData);
